...
public class ParameterController {
    ...
    public void paraHandeler_1(@RequestParam(name="username",required = true) String name){
        log.info("GET /param/p01...." + name); // 윗줄 파라미터 지정
    }

    @RequestMapping("/p02")
    public void paraHandeler_2(@RequestParam(name="username",required = true) String name){
        log.info("GET /param/p02...." + name);
    }

    @PostMapping("/p03")
    public void paraHandeler_3(@RequestParam(name="username",required = true) String name){
        log.info("POST /param/p03...." + name);
    }
//    @RequestParam : 동기요청 파라미터 처리 / html form 기반 전달되는 파라미터 처리(JS의 form-data도 받기가능, JSON Type 받기불가)
//    @RequestBody : 비동기요청 파라미터 처리 / json,filedata등 전달되는 파라미터를 처리
    @PostMapping("/p04")
    public void paraHandeler_4(@RequestBody String name){
        log.info("POST /param/p04...." + name);
    }

    @PostMapping("/p05")
    public void paraHandeler_5(@RequestParam(name="username",required = false, defaultValue = "홍길동") String name){
        log.info("POST /param/p05...." + name);
    }

    @GetMapping("/p06")
    public void paraHandeler_6(
    ...
    @GetMapping("/p07")
    public void paraHandeler_7(/*@ModelAttribute 생략가능*/ PersonDto dto){
        log.info("GET /param/p07...."+dto);
    }

    @GetMapping("/p08/{name}/{age}/{addr}")
    public void paraHandeler_8(
    ...
    public void paraHandeler_9(PersonDto dto){
        log.info("GET /param/p09...." +dto);

    }

    @GetMapping("/page1")
    public void page1(PersonDto dto, Model model){ // Model : 화면에 데이터 출력
    ...
    public ModelAndView page2_handler(PersonDto dto){
        ...
        ModelAndView modelAndView = new ModelAndView(); // 이동할 뷰 경로 지정할 수 있는 객체
    ...
    public void page3_handler(HttpServletRequest request, HttpServletResponse response){
        ...
        PersonDto dto = new PersonDto(name,age,addr); // persondto 객체생성
...
//        response.addCookie(cookie);
    ...
    @GetMapping("/page4")
    public void page4_handler(@RequestParam Map<String,Object> params) // 중요
    {
    ...
    public String redirect_init_handler(Model model, RedirectAttributes redirectAttributes){
        ...
        model.addAttribute("init","init_value"); // 쿼리스트링
    ...
    public void redirect_step1_handler(Model model){
